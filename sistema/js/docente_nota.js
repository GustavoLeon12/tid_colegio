 ...
 function listar_Alumnos(idgrado,idnivel,idyear,idsecion){
    $.ajax({
     ...
     alumnos = JSON.parse(resp);
      if (alumnos.length==0) {
         $("#buttNew").hide();
         return Swal.fire("Mensaje de Advertencia", "No se encontro ningun alumno matrículado en el grado,nivel,seccioón y año especificado  !!", "warning");
  ...
  var idcurso=cursoid;
    $.ajax({
     ...
     if (actividades.length==0) {
       ...
       return Swal.fire("MENSAJE DE ADVERTENCIA", " El curso seleccionado no tiene cargas académicos para el tipo de  (NF/BIM/TRIM/SEM) SELECCIONADO ,NO HAY NADAD QUE EVALUAR !!", "warning");
