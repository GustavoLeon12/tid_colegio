...
class Modelo_Usuario
{
  ...
  function Registrar_Docente($id_docente, $nombre, $apellido, $usuario, $contra, $cbm_rol, $NameImg, $dni, $tp_docu)
  {
    $id_docente = $id_docente ? $id_docente : '0';
    $sql = "INSERT INTO usuarios(identidad,usu_usuario,usu_nombre,usu_apellidos,usu_contrasena,rol_id,imagen, dni, tp_docu)
     VALUES ('$id_docente','$usuario','$nombre','$apellido','$contra','$cbm_rol','$NameImg', '$dni', '$tp_docu')";
    if ($consulta = $this->conexion->conexion->query($sql)) {
      return 1;
    } else {
      return 0;
  ...
  function Registrar_Alumno($id_alumno, $nombre, $apellido, $usuario, $contra, $cbm_rol, $NameImg, $dni, $tp_docu)
  {
    ...
    $sql = "INSERT INTO usuarios(identidad,usu_usuario,usu_nombre,usu_apellidos,usu_contrasena,rol_id,imagen, dni, tp_docu)
   VALUES ('$id_alumno','$usuario','$nombre','$apellido','$contra',' $cbm_rol','$NameImg', '$dni', '$tp_docu')";
    if ($consulta = $this->conexion->conexion->query($sql)) {
      return 1;
    } else {
      return 0;
  ...
  function Registrar_Usuario($id_otro, $nombre, $apellido, $usuario, $contra, $cbm_rol, $NameImg, $dni, $tp_docu)
  {
    $id_otro = $id_otro ? $id_otro : '0';
    $sql = "INSERT INTO usuarios(identidad,usu_usuario,usu_nombre,usu_apellidos,usu_contrasena,rol_id,imagen, dni, tp_docu)
   VALUES ('$id_otro','$usuario','$nombre','$apellido','$contra',' $cbm_rol','$NameImg', '$dni', '$tp_docu')";
    if ($consulta = $this->conexion->conexion->query($sql)) {
      return 1;
    } else {
      return 0;
  ...
  {
    $sql = " select apellidos, alumnonombre,dni,rolalumno,alumno_foto from alumno where bajaAlumn='1' and idalumno='$idalumno'";
    $arreglo = array();
    if ($consulta = $this->conexion->conexion->query($sql)) {
      while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
        $arreglo[] = $consulta_VU;
  ...
  {
    $sql = " select apellidos, nombres,dni,rolDocente, docente_foto from docentes where estado_baja='1' and id_docente='$iddocente'";
    $arreglo = array();
    if ($consulta = $this->conexion->conexion->query($sql)) {
      while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
        $arreglo[] = $consulta_VU;
