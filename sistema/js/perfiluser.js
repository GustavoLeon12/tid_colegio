function Extraer_Datos_Perfil_user() {
  $.ajax({
    ...
    if (data.auth) {
      ...
      $("#mostrarimagen").attr("src", data[0]["imagen"]);
      $("#datos_usuario").html(
        data[0]["usu_nombre"] + "," + data[0]["usu_apellidos"]
      );
      ...
      if (data.data > 0) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            window.location =
              "../controlador/usuario/controlador_cerrar_session.php";
          }
          location.reload();
        });
      } else {
