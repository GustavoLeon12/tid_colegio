...
async function Listar_combo_gradosAsistencia_by() {
  console.log("Alex enseñame a ser senior");
  var identi = "";
  var nameCombo = "--seleccione--";
  $.ajax({
    url: "../controlador/notas/controlador_combo_grados.php",
...
async function lista_combo_filtrados_by(id) {
  let desarrolladores = temturno.filter((item) => item.idgrado == id);
  ...
  $("#nivelIdHidden").val(desarrolladores[0]["idgrado"]);
  $("#gradoIdHidden").val(desarrolladores[0]["nivel_id"]);
  $("#seccionIdHidden").val(desarrolladores[0]["seccion"]);
}
...
async function ShowSelectedBy() {
  var idgrado = $("#cbm_grado_by").val();
  if (idgrado) {
    lista_combo_filtrados_by(idgrado);
  }
}
...
async function lista_combo_filtrados_Elimiar(id) {
  ...
  $("#elim_txt_nivel_nivel_seccion").val(
    desarrolladores[0][3] + " | " + desarrolladores[0][4]
  );
}
...
function Listar_Alumno_Asistencia() {
  ...
  if (idgrado?.length == 0 || idsecion?.length == 0 || idnivel?.length == 0) {
    return Swal.fire(
      ...
      "Debes seleccionar los prámetos para listar alumnos matriculados  !!",
      "warning"
    );
  }
  ...
  $.ajax({
    ...
    if (data.length != 0) {
      ...
      var datos_add = "";
      datos_add +=
        "<tr><td ></td><td ></td><td >No se encontó Ningun Alumno Matriculado para los parametros ingresados!</td><tr>";
...
function Listar_Asistencias_fecha() {
  ...
  if (fechaEntrada.length == 0) {
    return Swal.fire(
      "Mensaje de advertencia",
      "Ingrese la fecha que deseas editar !!",
      "warning"
    );
  }
  ...
  $.ajax({
    ...
    if (data?.length != 0) {
      ...
      var datos_add = "";
      datos_add +=
        "<tr><td ></td><td ></td><td >No hay asistencia registrado para la fecha ingresado!</td><tr>";
...
function Estraer_Asistencia_Eliminar() {
  ...
  if (idgrado?.length == 0 || idsecion?.length == 0 || idnivel?.length == 0) {
    return Swal.fire(
      "MENSAJE DE ADVERTENCIA",
      "Debes seleccionar los prámetos para listar Asistencias  !!",
      "warning"
    );
  }
  if (fechainicio.length == 0 || fechafin.length == 0) {
    return Swal.fire(
      "Mensaje de advertencia",
      "Ingrese la fecha que deseas Visualizar !!",
      "warning"
    );
  }
...
function Update_Asistencia() {
  ...
  if (fechaAsisten.length == 0) {
    return Swal.fire(
      "Mensaje De Advertencia",
      "Ingrese fecha de las asistencias",
      "warning"
    );
  }
  ...
  $.ajax({
    ...
    type: "POST",
    data: {
      vectorIdpersonas: vectorIdpersonas,
      vectorEstado: vectorEstado,
      fechaAsisten: fechaAsisten,
    },
  }).done(function (resultado) {
    var data = JSON.parse(resultado);
      ...
      $.ajax({
        ...
        if (data != null) {
          ...
          Swal.fire({
            icon: "success",
            title: "Mensaje de Éxito !!",
            text: "Datos correctamente, Las Asistencias Se eliminó Corectamente",
            showConfirmButton: false,
            timer: 1500,
          });
          Black_MenuAsis();
          ...
          return Swal.fire(
            "Mensaje De Advertencia",
            "No se pudo Eliminar  las asistencias" + resultado,
            "error"
          );
        }
      });
...
function RegistrarAsistencia() {
  ...
  $.ajax({
    ...
    if (data.status) {
      ...
      return Swal.fire(
        "Mensaje De error",
        "No se pudo registrar las asistencias" + data.msg,
        "error"
      );
    }
  });
...
function recorerresultado_Asistencias(data) {
  ...
  data.forEach((valor) => {
    ...
    datos_add +=
      "<input type='checkbox' id='new_comboAsistencia' class='clas_chebo" +
      n +
      "'>";
...
function Asistencias_Encontrados(data) {
  ...
  data.forEach((valor) => {
    ...
    datos_add +=
      ...
      "</td>";
    datos_add +=
      "<td><label class='switch_checbok' style='display: block !important;'>";
    valor.Est_Asis == 1
      ? (datos_add +=
          "<input type='checkbox' id='edit_comboAsistencia' checked>")
...
function Recorer_Filtros_Asistencia(data) {
  ...
  data.forEach((valor) => {
    ...
    valor.Est_Asis == 1
      ? (datos_add += "<span class='label label-success'>Asistió</span>")
...
function Estraer_Lista_Range_Alum() {
  ...
  var nivelid = $("#nivelIdHidden").val();
  var gradoid = $("#gradoIdHidden").val();
  var seccionid = $("#seccionIdHidden").val();
