...
function Limpiar_Registrar_Usuario() {
  ...
  $("#perfil_seleccionararchivo").prop("disabled", false);
...
function Estraer_Datos_Alumno() {
  ...
  $.ajax({
    ...
    if (data.length > 0) {
      ...
      $("#perfil_seleccionararchivo").prop("disabled", true);

      $("#image_hidden").val(data[0]["alumno_foto"]);
...
function Estraer_Datos_Docentes() {
  ...
  $.ajax({
    ...
    if (data.length > 0) {
      ...
      $("#image_hidden").val(data[0]["docente_foto"]);
      $("#txt_apellido").val(data[0]["apellidos"]);
      $("#txt_dni").val(data[0]["dni"]);
...
function Registrar_Usuario() {
  ...
  formData.append("image", ImgPerfil);
  formData.append("imageHidden", image_hidden);
...
function ResquestHttp(resp, nombre, apellido) {
  if (resp > 0) {
    ...
    if (resp == 100) {
      ...
      return Swal.fire(
        "Mensaje De Advertencia",
        "El Usuario " + apellido + ", " + nombre + " ya esta registrado !!!",
        "warning"
      );
    }
    if (resp == 404) {
    ...
    return Swal.fire(
      "Mensaje De Error",
      "No se registro Registro Fallido!!" + resp + "",
      "error"
    );
  }
}
      ...
      $.ajax({
        ...
        if (resp > 0) {
          ...
          Swal.fire({
            icon: "success",
            title: "Éxito !!",
            text: "La operación, se registro  de forma Éxitoso!! ",
            showConfirmButton: false,
            timer: 1700,
          });
        } else {
          Swal.fire({
            icon: "info",
            title: "No se Completo !!",
            text: "El usuario tiene la sesion Activa!! ",
            showConfirmButton: false,
            timer: 1700,
          });
        }
  ...
  $.ajax({
    ...
    if (resp > 0) {
      ...
      Swal.fire({
        icon: "success",
        title: "Éxito !!",
        text: "El Registro, se registro  de forma Éxitoso!! " + mensaje,
        showConfirmButton: false,
        timer: 1700,
      });
    } else {
...
function XMLHttpRequestAsycn(request) {
  ...
  /*  if (request==403) {
        ...
        if (request.length>0) {
            ...
            Generar_Session_Usuario(request);
        }
         if (request==404) {
            window.location = "NotFound";
        }
