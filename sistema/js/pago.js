...
function listar_Alumno_Pagos() {
  ...
  table_pagos = $("#Tabla_Pagos_Alumno").DataTable({
    ...
    columns: [
      ...
      {
        defaultContent:
          "<button  type='button' class='pagar btn btn-primary btn-sm'><i class='fa fa-money' title='pagar'></i></button>" +
...
function Agregar_tabla_Pagos() {
  ...
  if (MesUtimoPago == 12) {
    ...
    var event = new Date(
      fechaparaBD.setFullYear(fechaparaBD.getFullYear() + 1)
    );
    var date = JSON.stringify(event);
  ...
  if (fechaSelec < f) {
    ...
    if (
      fechaSelec.getMonth() == f.getMonth() &&
      fechaSelec.getFullYear() == f.getFullYear()
    ) {
      createNotification(
        "LOS PAGOS SE ACEPTAN PASADO 1 MES DE LA  FECHA (" +
          fechpagado +
          ") .... ",
        "info"
      );
    ...
    if (verificarid(mesSelect)) {
      createNotification(
        "EL MES " + selectFecha + " YA ESTA SELECCIONADO PARA SU PAGO",
        "info"
      );
      return;
    }
    ...
    if (precio.length == 0) {
      createNotification("INGRSE MONTO", "error");
      return;
    }
    var datos_add = "<tr>";
    datos_add += "<td for='id'>" + mesSelect + "</td>";
    datos_add +=
      "<td > <strong>" + selectFecha.toUpperCase() + "</strong></td>";
    datos_add += "<td hidden >" + fechaSelecnormal + "</td>";
    fechaSelecnormal = null;
    datos_add += "<td >" + precio + "</td>";

    datos_add +=
      "<td><button class='btn btn-secondary btn-sm' onclick = 'remove(this)' ><em class='fa fa-close'></em></button><br> </td>";
    datos_add += "</tr>";
  ...
  if (Request > 0) {
    ...
    if (Request == 1) {
      ...
      Swal.fire({
        icon: "success",
        title: "Éxito !!",
        text: "Los Pagos se registraron con Exito!!!",
...
function Imprimir_Boleta_De_Pago() {
  ...
  var yearid = $("#YearActualActivo").val();
  window.open(
    "../vista/reportePDF/vista_imprimir_Boleta_pago.php?alumid=" +
      alumid +
      "&yearid=" +
      yearid +
      "#zoom=75%",
    ...
    if (datos.length > 0) {
      datos.forEach((valor, i) => {
        ...
        html +=
          ...
          Number(1 + i) +
          ").&nbsp;<a>" +
          formatoFechas(valor.fechasPagados) +
          "</a></b> <a class='pull-right'>" +
          valor.motoPago +
          "</a>";
        html += "</li>";
        html += "</ul>";
...
function listar_Reportepago_masPagos_realizados(id) {
  ...
  $("#tabla_meses_pagado").html(
    "<br> <center> REALIZA TU PRIMER PAGO !!</center>"
  );
}
