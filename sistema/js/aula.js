    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (data == "LIBRE") {
            return (
              "<span class='label label-success'><i class ='fa fa-info-circle'></i>&nbsp;" +
              data +
              "</span>"
            );
          } else {
      ...
      {
        defaultContent:
          "<button  type='button' class='editar btn btn-primary btn-sm'><i class=' fa fa-edit' title='ediat'></i></button>&nbsp;<button  type='button' class='eliminar btn btn-default btn-sm title='eliminar'><i class='fa fa-trash'></i></button>",
      },
    ],
...
$("#tabla_aulas").on("click", ".eliminar", function () {
  ...
  $.ajax({
    ...
    if (resp > 0) {
      ...
      Swal.fire(
        "Mensaje De Error",
        "no se pudo completr,ya esta ocupado poe un grado",
        "error"
      );
    }
  });
