...
function listar_Alumnos(idgrado, idnivel, idyear, idsecion) {
  $.ajax({
    ...
    //guardar alumnos en un vector
    alumnos = JSON.parse(resp);
    if (alumnos.length == 0) {
      $("#buttNew").hide();
      return Swal.fire(
        "MENSAJE DE ADVERTENCIA",
        "NO SE ENCONTRO NINGUN ALUMNO MATRICULADO EN EL GRADO,NIVEL,SECCIÓN Y AÑO ESPECIFICADO  !!",
        "warning"
      );
    } else {
    }
...
function listar_Actividades_curso(cursoid, tipoorden, tipoid, idyear) {
  var idcurso = cursoid;
  $.ajax({
    ...
    if (actividades.length == 0) {
      ...
      return Swal.fire(
        "MENSAJE DE ADVERTENCIA",
        " El curso seleccionado no tiene cargas académicos para el tipo de  (NF/BIM/TRIM/SEM) SELECCIONADO ,NO HAY NADAD QUE EVALUAR !!",
        "warning"
      );
    } else {
    }
...
async function ListarComboDePeriodoDeEvaluacionYearActivoYFechas() {
  ...
  $.ajax({
    ...
    if (data.length > 0) {
      ...
      cadena +=
        "<option value=''>NO SE ENCONTRARON AÑOS ACADEMICOS ACTIVOS</option>";
      $("#cbm_tipoOrden").html(cadena);
    }
...
function EtraerDatosSegunLosParametrosEstablecidos() {
  ...
  if (
    ...
    return Swal.fire(
      "Mensaje de advertencia",
      "Debes seleccionar los paramentros para generar o mostrar notas  !!",
      "warning"
    );
  }
...
function notas(j, id) {
  ...
  resul[j]?.nota_alum
    ...
    : notasTem.push("0.00");
  //notasTem.push(resul[j].nota_alum);
  return resul[j] != null ? resul[j].nota_alum : 0.0;
}
...
function Listar_notas_Alumnos() {
  if (actividades === "undefined") {
    console.log(actividades + "Actividades undefined");
  ...
  if (actividades?.length === 0) {
    console.log(actividades + "actividades?.length===0");
    //EtraerDatosSegunLosParametrosEstablecidos(); //CHEK
  }
  ...
  $.each(actividades, function (i, acti) {
    html +=
      "<th hidden><input type='text' id='activityid' value='" +
      acti.actcur_id +
      "'></th>";
    html +=
      "<th>" + acti.actividades + "&nbsp;(" + acti.puntajes + "%)" + "</th>";
  });
  //---FINAL---//
  ...
  $.each(alumnos, function (i, alum) {
    ...
    $.each(actividades, function (j, nota) {
      ...
      html +=
        "class=' form-control ' id='text_nota' value='" +
        notas(j, alum.idalumno) +
        "' >";
      html += "</td>";
    });
    //---FINAL--//
    html +=
      "<td ><input type='number' name='acumulados' class='form-control' id='acumulado" +
      i +
      "' disabled value='" +
      promedio() +
      "'></td>";
...
function Guardar_Registro_Notas() {
  ...
  if (
    ...
    return Swal.fire(
      "Mensaje de advertencia",
      "Es necesario seleccionar curso,nivel,seccion, tipo (NF/BIM/TRIM/SEM) que son requeridos para la corecta configuración",
      "warning"
    );
  }
  ...
  $("#table_notas .trcontext td input[name='acumulados'] ").each(function (
    j,
    item
  ) {
  ...
  $.ajax({
    ...
    data: {
      ...
      idgrado: idgrado,
      idsecion: idsecion,
      idalumnos: idalumnos.toString(),
      idactividad: idactividad.toString(),
      notas: notas.toString(),
      ponderados: ponderados.toString(),
    },
  ...
  if (Request > 0) {
    if (Request == 1) {
      ...
      Swal.fire({
        icon: "success",
        title: "Éxito !!",
        text: "Se registro corectamente las notas para los parametros especificados !!",
    ...
    if (Request == 100) {
      ...
      return Swal.fire(
        "Mensaje De Advertencia",
        "El Registro Similar(Igual) a esto ya  Existe...sugerencia Puedes Editar si desear modificar ,Mientras no hay evaluacines Activas ",
        "warning"
      );
    }
    if (Request == 404) {
...
function onclickhandler(i) {
  ...
  $("#acumulado" + i).val(acumuado == 0 ? 0.0 : acumuado);
}

//LISTAR CURSOS PERTENECENTES AL GRAADO Y ALUMUNOS DE ESE GRADOS
async function listar_Combo_Cursos_grado(idgrado) {
  var gradoid = idgrado;
