...
<?php
    ...
    $Exist=$MU->Verificar_Existencia($alumno,$yearid);
    if($Exist==0){
          $consulta = $MU->Registrar_New_Matricula($alumno,$grado,$pago,$aula,$turno,$nivel,$yearid,$cargo,$seccion);

           if($cargo=='SI'){
            $consulta = $MU->Registrar_Pagos_de_Penciones($alumno,$pago,$yearid,$fecha_actual,$fechmas);

            $consulta = $MU->Registrar_Pagos_stado_de_pagos($alumno,$yearid,$fecha_actual,$fechmas,$cargo);
            }
