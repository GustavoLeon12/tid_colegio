var editando = false;
async function listar_areas() {
  const res = await fetch("../controlador/curso/controlador_listar_cat.php");
  ...
  categories.forEach((element) => {
    area.innerHTML += `
    <option value="${element.id}">${element.nombrecat}</option>
    `;
  });
}
listar_areas();
...
function Registrar_curso() {
  ...
  $.ajax({
    url:
      editando === false
        ? "../controlador/curso/controlador_registrar_curso.php"
        : "../controlador/curso/controlador_curso_Actualizar.php",
    type: "POST",
    data: {
      idcurso: idcurso,
      codigocur: codigocur,
...
$("#tabla_curso").on("click", ".eliminar", function () {
  ...
  $.ajax({
    ...
    if (resp > 0) {
      ...
      return Swal.fire(
        "Mensaje De Advertencia",
        "ups !,no se pudo eliminar, curso esta asignado a , grado , horario o docente",
        "warning"
      );
    }
  });
