...
function Registrar_Grado() {
  ...
  var nomb = $("#txt_nom").val().toUpperCase();
  ...
  $.ajax({
    url:
      editando === false
        ? "../controlador/grados/controlador_registrar_grado.php"
        : "../controlador/grados/controlador_editar_grado.php",
    type: "POST",
    data: {
      idGrado: idGrado,
      nomb: nomb,
...
function listar_grados() {
  table_grado = $("#tabla_grados").DataTable({
    ...
    columns: [
      ...
      {
        defaultContent:
          "<button type='button' class='edit btn btn-primary btn-sm'><en class=' fa fa-edit' title='edit'></en></button>&nbsp;<button  type='button' class='eliminar btn btn-default btn-sm' title='eliminar'><em class='fa fa-trash'></em></button>",
      },
    ],
...
function Eliminar_Grado(idgrado) {
  $.ajax({
    ...
    if (resp > 0) {
      ...
      return Swal.fire(
        "Mensaje De Advertencia",
        "Lo sentimos No se pudo eliminar,Elgrado esta Activo,El grado esta siendo usado en Alumnos,Pagos,Horarios,Docente,Cursos,Primero quita de estas opciones",
        "warning"
      );
    }
  });
