...
class Asistensia
{
 ...
 WHERE (Fechas >= '$fechainicio' AND Fechas <= '$fechafin') and idgrado='$idgrado' and idnivel='$idnivel' and idseccion='$idsecion' and yearid='$idyear' ";
  ...
  function Extraer_asistencias($fechainicio, $fechafin, $grado, $nivel, $seccion)
  {

    $sql = " select idalumno,apellidos,alumnonombre,Fechas,stadoalumno,Est_Asis from asistencia
  inner join  alumno on alumno.idalumno= asistencia.idalumno_asi
 WHERE (Fechas >= '$fechainicio' AND Fechas <= '$fechafin') AND idgrado='$nivel' AND idnivel='$grado' AND idseccion='$seccion'";
    $arreglo = array();
    if ($consulta = $this->conexion->conexion->query($sql)) {
      while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
  ...
  function verificar_Asistencia($fechas, $grado, $nivel, $seccion)
  {
    $sql = "select * from asistencia where Fechas='$fechas' and idgrado='$grado' and idnivel='$nivel' and idseccion='$seccion'";
    $arreglo = array();
